      ...
      <div style={{ background: '#020a14', border: '1px solid #0d2035', borderRadius: 6, overflow: 'hidden', marginBottom: 8 }}>
        ...
        {localDev.grid.map((row, ri) => (
          <div key={ri} style={{ display: 'grid', gridTemplateColumns: 'repeat(9,1fr)', borderBottom: ri < 2 ? '1px solid #0d2035' : 'none' }}>
            {row.map((cell, ci) => (
              <div key={ci} style={{
                height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRight: ci < 8 ? '1px solid #0d2035' : 'none',
...
function HackingDevice({ device, currentNum, clickWindowOpen, calledNums, onCellClick, onClaim, onActivate, winStates, bankruptCount, timer, totalTimer, liveBank, onOpenVault }: {
  ...
  return (
    <div style={{
      background: 'linear-gradient(180deg,#0d1a2e,#060e1a)', border: `2px solid ${canClaim ? '#ec4899' : bestPct > 0.5 ? proxColor : device.active ? '#00e5a030' : '#162438'}`, borderRadius: 14, padding: 0,
        ...
        <div style={{ position: 'relative', flex: 1, display: 'flex', margin: '0 2px' }}>
          <button onClick={doClaim} disabled={!canClaim} style={{
            ...
            <span style={{
              ...
              textShadow: canClaim ? '0 0 8px #ff2020,0 0 20px #ff202080' : bestPct > 0.6 ? `0 0 6px ${proxColor}` : 'none'
            }}>RANSOM</span>
          </button>
          {onOpenVault && (
    ...
    <div style={{ background: '#020d1a', border: '2px solid #0a3a5a', borderRadius: 14, overflow: 'hidden', display: 'flex', position: 'relative', minHeight: 360 }}>
                ...
                {lastNum !== null ? (
                  <>
                    ...
                    {glitching && (<>
                      ...
                      <div style={{
                        position: 'absolute', inset: 0, fontFamily: 'Syne,sans-serif', fontSize: 72, fontWeight: 800,
                        color: '#00b8ff', opacity: 0.5, animation: 'glitchB 0.6s ease', pointerEvents: 'none'
                      }}>{lastNum}</div>
                    </>)}
                  </>
                ) : (
      ...
      <div className="matrix-right-panel" style={{ width: 170, flexShrink: 0, padding: '10px 10px', zIndex: 2, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ height: 4, background: '#0a1628', borderRadius: 2, overflow: 'hidden', border: '1px solid #0d2035' }}>
          <div style={{
            height: '100%', width: `${vaultPct * 100}%`, background: 'linear-gradient(90deg,#00e5a0,#22c55e)',
  ...
  if (phase === 'lobby') return (
    <div data-theme={theme} style={{ minHeight: '100vh', background: '#050f17', color: '#dce6f3', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', paddingTop: 56 }}>
        <div className={`lobby-sidebar${mobileMenuOpen ? ' mobile-open' : ''}`} style={{ width: 200, minHeight: 'calc(100vh - 56px)', background: '#09141e', borderRight: '1px solid rgba(0,229,160,0.08)', display: 'flex', flexDirection: 'column', padding: '16px 0', flexShrink: 0 }}>
    ...
    <div data-theme={theme} style={{ background: 'linear-gradient(180deg,#010810,#020d1a)', color: '#c8d8e8', minHeight: '100vh' }}>
      ...
      {showTerminate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          ...
          <div style={{
            background: '#0a0505', border: '2px solid #7f0000', borderRadius: 16, padding: 28, maxWidth: 360, width: '100%',
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setShowTerminate(false)}
                style={{
                  flex: 1, background: '#0a1628', border: '1px solid #1e3a5f', color: '#4a7fa5', borderRadius: 8,
