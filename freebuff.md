...
### Session: 2026-09-19 — Round 3: Photoreal keycaps + ambient backdrop (match the photo exactly)
- **Backdrop:** body now renders the photo's ambient desk lighting — deck base `#080e14` with fixed radial cyan glow pools (top screen-spill `rgba(92,207,230,0.09)`, corner pools `rgba(70,140,175,0.11)`/`rgba(45,95,130,0.09)`) over a vertical deck gradient. Verified: base `rgb(8,14,20)` + 4 layers, `background-attachment: fixed`.
- **Keycaps rebuilt photoreal (`.keyboard-key`):** concave dish face (radial gloss sheen over 3-stop steel-blue surface `#47637a→#2c4254→#22364a`), directional side walls (lit top edge `rgba(185,225,240,0.30)`, dark right, 6px front wall `#101c28`), cyan backlit legends (`text-shadow 0 0 7px rgba(92,207,230,0.80)`), desk shadow + backlit underglow (`0 0 16px rgba(92,207,230,0.12)`). Travel: hover 1px dip (underglow strengthens), press bottoms out `top:5px` + 1px wall + LED bloom `0 0 26px rgba(92,207,230,0.30)` + inset shadow.
- **Accent/danger caps upgraded to backlit builds too:** cyan caps with white glow legends + `0 0 22px` underglow; crimson caps with red legend glow. Full hover/active states with `!important` on press bloom so the global rule can't flatten it.
- **Cascade fixes:** global `button:active` wrapped fully in `:where()` (specificity 0,1,0 — `:not()` counts); removed stale duplicate `.keyboard-key-accent:hover/:active` rules (old `top:4px` flat-active was overriding the new backlit active — the travel read 4px instead of 5px).
- All 3 states re-verified computed-style: hover 1px → press 5px/1px + bloom. Behavior regressions pass (caliber keys, glider slide, matrix entry). tsc 0, build passes, dev server 200.

### Session: 2026-09-19 — Round 4: FULL NEON (purple + white) — user directive
- **Direction change:** user asked for all-neon: purple ambient background with white core light, keys as glowing neon lights, whole site glows. No image constraint anymore.
- **Backdrop:** void base `#0a0612` with purple wash pools (`rgba(168,85,247,0.16/0.20/0.16)` top/bottom corners) + **white core halo** `rgba(255,255,255,0.05)` mid-screen, fixed attachment.
- **Hue sweep (~450 occurrences):** cyan family → purple (`#b26bff`, `#c084fc`, `#8b5cf6`, `#7c3aed`, `#a855f7`), all steel-blue/navy surfaces → deep violet-slate (`#150d24`, `#1a1029`, `#1f1330`, `#241538`), remaining matrix-screen blues swept to violet, crimson → neon pink `#ff5c8a`/`#e0246a`, ambers/pinks kept as semantic LEDs.
- **Neon keys:** every keycap state rebuilt as a glowing purple tube with white core — face `#3b2465→#241345` + white sheen, halo `0 0 18px rgba(168,85,247,0.45)`, inner white light `inset 0 0 12px`, white legend glow `text-shadow 0 0 8px #fff`. Accent = fully lit purple cap (white-hot core). Danger = neon pink tube. Press = cap bottoms out and flares: `0 0 40px rgba(192,132,252,0.65)` + `0 0 80px` outer bloom + white inner flare.
